...
import com.trayis.geocache.Node;
import com.trayis.geocache.NodeType;
...
public class GeoChacheTest extends junit.framework.TestCase {

    private GeoChache getTree() {
        GeoChache qt = new GeoChache(0, 0, 100, 100);
        qt.set(5, 20, "Foo");
        qt.set(50, 32, "Bar");
        qt.set(47, 96, "Baz");
        qt.set(50, 50, "Bing");
    ...
    private void assertTreesChildrenAreNull(GeoChache qt) {
    ...
    public void testSearchIntersects() {
        GeoChache qt = getTree();
        Location[] points = qt.searchIntersect(4, 0, 51, 98);
        Arrays.sort(points);
        String keyString = Arrays.asList(points).toString();
        String expected = "[(5.0, 20.0), (12.0, 0.0), (47.0, 96.0), (50.0, 32.0), (50.0, 50.0)]";
        assertEquals("Sorted keys should be " + expected, expected, keyString);

        Location[] points2 = qt.searchIntersect(5, 0, 50, 96);
        Arrays.sort(points2);
        ...
        Location[] points3 = qt.searchIntersect(55, 0, 50, 96);
    ...
    public void testSearchWithin() {
        GeoChache qt = getTree();
        Location[] points = qt.searchWithin(4, -1, 51, 98);
        Arrays.sort(points);
        String keyString = Arrays.asList(points).toString();
        String expected = "[(5.0, 20.0), (12.0, 0.0), (47.0, 96.0), (50.0, 32.0), (50.0, 50.0)]";
        assertEquals("Sorted keys should be " + expected, expected, keyString);
    ...
    public void testConstructor() {
        GeoChache qt = new GeoChache(-10, -5, 6, 12);
        Node root = qt.getRootNode();
    ...
    public void testBalancing() {
        ...
        assertEquals("first", root.getPoint().getData());
    ...
    public void assertFails(GeoChache qt, double[] args) {
        try {
