...
public class GeoChache {
     ...
     */
    public GeoChache(double minX, double minY, double maxX, double maxY) {
    ...
    public void set(double x, double y, Object value) {
        ...
        if (this.insert(root, new Location(x, y, value))) {
            this.count++;
        }
    }
    ...
    public Object get(double x, double y, Object opt_default) {
        ...
        return node != null ? node.getPoint().getData() : opt_default;
    }
    ...
    public Object remove(double x, double y) {
        Node node = this.find(this.root, x, y);
        if (node != null) {
            Object value = node.getPoint().getData();
            node.setPoint(null);
    ...
    public Location[] getKeys() {
        final List<Location> arr = new ArrayList<Location>();
        this.traverse(this.root, new Func() {
            @Override
            public void call(GeoChache quadTree, Node node) {
                arr.add(node.getPoint());
            }
        });
        return arr.toArray(new Location[arr.size()]);
    }
    ...
    public Location[] searchIntersect(final double xmin, final double ymin, final double xmax, final double ymax) {
        final List<Location> arr = new ArrayList<Location>();
        this.navigate(this.root, new Func() {
            @Override
            public void call(GeoChache quadTree, Node node) {
                Location pt = node.getPoint();
    ...
    public Location[] searchWithin(final double xmin, final double ymin, final double xmax, final double ymax) {
        final List<Location> arr = new ArrayList<Location>();
        this.navigate(this.root, new Func() {
            @Override
            public void call(GeoChache quadTree, Node node) {
    ...
    public GeoChache clone() {
        ...
        this.traverse(this.root, new Func() {
            @Override
            public void call(GeoChache quadTree, Node node) {
                clone.set(node.getPoint().getX(), node.getPoint().getY(), node.getPoint().getData());
            }
        });
     ...
     * @private
     */
    private boolean insert(Node parent, Location point) {
    ...
    private void split(Node node) {
        Location oldPoint = node.getPoint();
        node.setPoint(null);
     ...
     * @private
     */
    private void setPointForNode(Node node, Location point) {
